<template>
    <div class="p-6 bg-gray-50 text-medium text-gray-500 dark:text-gray-400 dark:bg-gray-800 rounded-lg w-full">
        <div data-dial-init class="fixed bottom-6 end-6 group z-50">
            <div id="speed-dial-menu-text-outside-button-square"
                class="flex flex-col items-center hidden mb-4 space-y-2">
                <button type="button" data-modal-toggle="upload-modal" data-modal-target="upload-modal"
                    ...
                    <span class="absolute block mb-px text-sm font-medium -translate-y-1/2 -start-14 top-1/2">上傳</span>
                </button>
                <button type="button" data-modal-toggle="edit-modal" data-modal-target="edit-modal"
                    ...
                    <span class="absolute block mb-px text-sm font-medium -translate-y-1/2 -start-14 top-1/2">修改</span>
                </button>
            </div>
        ...
        <!-- upload modal -->
        <teleport to="body">
            <div id="upload-modal" aria-hidden="true" data-modal-backdrop="static" @click="handleBackgroundClick"
                class="animate-fade-down hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                <div class="relative p-4 w-full max-w-md max-h-full transition-transform duration-1000 ease-in-out"
                    ref="modalPop">
                    <!-- Modal content -->
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <div
                            ...
                            <button type="button"
                                class="end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white font-mono"
        ...
        <!-- edit modal -->
        <teleport to="body">
            <div id="edit-modal" aria-hidden="true" data-modal-backdrop="static" @click="handleBackgroundClick"
                class="animate-fade-down hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                <div class="relative p-4 w-full max-w-md max-h-full transition-transform duration-1000 ease-in-out"
                    ref="modalPop">
                    <!-- Modal content -->
...
const handleBackgroundClick = (event: any) => {
    if (event.target.id === 'edit-modal' || event.target.id === 'upload-modal') {
        ...
        setTimeout(() => {
            modalPop.value.classList.remove('scale-105');
        }, 300);
    }
};
...
const handleUploadBook = async () => {
    ...
    try {
        //  成功上傳
        let response = await axios.post('https://yisinnft.org/api/book/' + signer + '/' + signature, formData, {
            headers: {
