    ...
    <div class="container max-w-[90vh]">
        <div
            ...
            <div class="rounded-md">
                <div class="text-3xl tracking-wide font-bold mb-4 text-gray-800">{{ showStore.bookInfo.title }}</div>
                <div class="text-lg tracking-wider font-medium">ISBN:{{ showStore.bookInfo.ISBN }}
                </div>
                <div class="text-lg tracking-wider font-medium">EISBN:{{ showStore.bookInfo.ISBN }}
                </div>
                <div class="text-lg tracking-wider font-medium">作者:{{ showStore.bookInfo.writer }}
                </div>
            ...
            <div class="rounded-md bg-blue-50/50 flex justify-center min-w-[70%] justify-self-end p-4">
                <div class="content-center">
                    <div class="text-2xl font-medium mb-2 flex justify-between">
                        <span class="text-sm">剩餘數量</span>{{ showStore.bookInfo.amount }}
                    </div>
                    <div class="text-2xl font-medium text-sky-600 mb-4 flex justify-between">
                        <span class="text-sm">優惠價</span>${{ showStore.bookInfo.price }}
                    </div>
                    <button type="button"
        ...
        <div class="text-sm font-medium text-gray-500 border border-gray-200 relative top-20 rounded-lg">
            ...
            <div id="default-tab-content" class="bg-blue-50/75">
                <div class="lg:ml-20 md:ml-8 sm:ml-0 hidden p-4 rounded-lg min-h-96 text-slate-700 text-lg tracking-wider font-medium grid grid-cols-2 gap-2 content-center"
                    ...
                    <div>EISBN:{{ showStore.bookInfo.ISBN }}</div>
                    <div>作者:{{ showStore.bookInfo.writer }}</div>
                    <div>出版社:{{ showStore.bookInfo.publisher }}</div>
                    <div>出版日期:{{ showStore.bookInfo.publishDate }}</div>
                    <div>上傳者:{{ showStore.bookInfo.uploader }}</div>
                    <div>種類:{{ showStore.bookInfo.class.className }}</div>
                    <div>年級:{{ showStore.bookInfo.class.grade }}</div>
                    <div>版本:{{ showStore.bookInfo.edition }}</div>
                    <div>互動書:{{ showStore.bookInfo.live }}</div>
                </div>
                <div class="hidden p-4 rounded-lg min-h-96 text-slate-700" id="introduction" role="tabpanel">
                    <p class="text-lg tracking-wider font-medium">{{
                        showStore.bookInfo.introduction }}</p>
                </div>
                <div class="hidden p-4 rounded-lg min-h-96 text-slate-700" id="chapter" role="tabpanel">
                    <p class="text-lg tracking-wider font-medium">{{ showStore.bookInfo.chapter }}
                    </p>
                </div>
            </div>
        </div>
